...
var CssComment_1 = require("./css-renderer/CssComment");
var CommentFactory = (function () {
    ...
    CommentFactory._simpleScrollingInitializer = function (manager, data) {
        var cmt = new Comment_1.ScrollComment(manager, data);
        switch (cmt.mode) {
            case 1: {
                cmt.align = 0;
                cmt.axis = 0;
    ...
    CommentFactory._simpleAnchoredInitializer = function (manager, data) {
        var cmt = new Comment_1.CoreComment(manager, data);
        switch (cmt.mode) {
            case 4: {
                cmt.align = 2;
                cmt.axis = 2;
    ...
    CommentFactory._advancedCoreInitializer = function (manager, data) {
        var cmt = new Comment_1.CoreComment(manager, data);
        cmt.init();
        cmt.transform = CommentUtils_1.CommentUtils.Matrix3D.createRotationMatrix(data.rX, data.rY, data.rZ).flatArray;
        manager.stage.appendChild(cmt.dom);
        return cmt;
    };
